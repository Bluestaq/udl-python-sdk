...
class Entity(BaseModel):
    """
    An entity is a generic representation of any object within a space/SSA system such as sensors, on-orbit objects, RF Emitters, spacecraft buses, etc. An entity can have an operating unit, a location (if terrestrial), and statuses.
    """
...
class Sensorcharacteristic(BaseModel):
    ...
    acceptable_sat_id_formats: Optional[List[str]] = FieldInfo(alias="acceptableSatIdFormats", default=None)
    """
    Array of acceptable satellite ID formats for sensor tasking and catalog
    transmission (e.g. 5-DIGIT, 9-DIGIT, ALPHA-5, INTERNATIONAL DESIGNATOR, etc.).
    These formats are not necessarily applicable to UDL sensor tasking.
    """
    ...
    consistent with sensor detection bands.
    """
    ...
    data_formats: Optional[List[str]] = FieldInfo(alias="dataFormats", default=None)
    ...
    """The beam width of a Sensor's Fan (range), in radians.
    ...
    max_data_transmission_rate: Optional[float] = FieldInfo(alias="maxDataTransmissionRate", default=None)
    ...
    max_num_tracks: Optional[int] = FieldInfo(alias="maxNumTracks", default=None)
    """Maximum number of simultaneous tracks.

    For phased array radars, this includes the capability to form multiple beams.
    For all sensors, it includes capability to distinguish multiple objects within a
    single beam or field-of-view.
    """
    ...
    The values for this range from 0.0 - + 99.99.
    """
    ...
    num_search_patterns: Optional[int] = FieldInfo(alias="numSearchPatterns", default=None)
    ...
    obs_per_track: Optional[int] = FieldInfo(alias="obsPerTrack", default=None)
    """Standard number of observations within a track."""

    obs_spacing: Optional[float] = FieldInfo(alias="obsSpacing", default=None)
    ...
    optical integration mode. The number of elements must be equal to the value
    indicated in numOpticalIntegrationModes.
    """
    ...
    search_pattern_points: Optional[List[float]] = FieldInfo(alias="searchPatternPoints", default=None)
    """
    Array of the actual search pattern points in the following order: time (in
    seconds), azimuth/right ascension (in degrees), elevation/declination (in
    degrees). The initial time is always 0, and subsequent times are in seconds
    since the first point.
    """

    settle_time: Optional[float] = FieldInfo(alias="settleTime", default=None)
    """
    ...
    tasking_channels: Optional[List[str]] = FieldInfo(alias="taskingChannels", default=None)
    """Array of available tasking channels (e.g.

    AD-HOC, CTL, DAILY, EVENT-ONLY, VOICE-ONLY, etc.).
    """
    ...
    """Performance zone-1 maximum range, in kilometers.
    ...
    """Performance zone-1 minimum range, in kilometers.

    Note that the zones apply only to the PSR/Search radars.
    """

    z2_max_range: Optional[float] = FieldInfo(alias="z2MaxRange", default=None)
    """Performance zone-2 maximum range, in kilometers.

    Note that the zones apply only to the PSR/Search radars.
    """

    z2_min_range: Optional[float] = FieldInfo(alias="z2MinRange", default=None)
    """Performance zone-2 minimum range, in kilometers.
