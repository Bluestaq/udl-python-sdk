# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from typing import List, Optional
from datetime import datetime
from typing_extensions import Literal, TypeAlias

from pydantic import Field as FieldInfo

from .._models import BaseModel

__all__ = [
    "CloselyspacedobjectTupleResponse",
    "CloselyspacedobjectTupleResponseItem",
    "CloselyspacedobjectTupleResponseItemCsoDetail",
]


class CloselyspacedobjectTupleResponseItemCsoDetail(BaseModel):
    object_event: str = FieldInfo(alias="objectEvent")
    """The type of object event the metrics apply. Values consist of START, END, MEAN."""

    object_type: str = FieldInfo(alias="objectType")
    """The type of the object for which the metrics apply.

    Values include PRIMARY, SECONDARY, DELTA.
    """

    id: Optional[str] = None
    """Unique identifier of the record, auto-generated by the system."""

    apogee: Optional[float] = None
    """The orbit point furthest from the center of the earth in kilometers."""

    classification_marking: Optional[str] = FieldInfo(alias="classificationMarking", default=None)
    """Classification marking of the data in IC/CAPCO Portion-marked format."""

    data_mode: Optional[Literal["REAL", "TEST", "SIMULATED", "EXERCISE"]] = FieldInfo(alias="dataMode", default=None)
    """Indicator of whether the data is REAL, TEST, EXERCISE, or SIMULATED data:

    REAL:&nbsp;Data collected or produced that pertains to real-world objects,
    events, and analysis.

    TEST:&nbsp;Specific datasets used to evaluate compliance with specifications and
    requirements, and for validating technical, functional, and performance
    characteristics.

    EXERCISE:&nbsp;Data pertaining to a government or military exercise. The data
    may include both real and simulated data.

    SIMULATED:&nbsp;Synthetic data generated by a model to mimic real-world
    datasets.
    """

    id_cso: Optional[str] = FieldInfo(alias="idCSO", default=None)
    """Unique identifier of the parent CSO record, auto-populated by the system."""

    inclination: Optional[float] = None
    """
    The angle between the equator and the orbit when looking from the center of the
    Earth in degrees from 0 to 180.
    """

    longitude: Optional[float] = None
    """The longitude degree of the object measured in degrees from -180 to 180."""

    perigee: Optional[float] = None
    """The orbit point nearest to the center of the earth in kilometers."""


class CloselyspacedobjectTupleResponseItem(BaseModel):
    classification_marking: str = FieldInfo(alias="classificationMarking")
    """Classification marking of the data in IC/CAPCO Portion-marked format."""

    cso_state: str = FieldInfo(alias="csoState")
    """
    Indicates the current state that characterizes Closely Spaced Objects (CSO)
    analysis steps and conclusions. Values include: ACTIVE, ACTUAL, CANCELED,
    CLOSED, COMPLETED, DETECTED, INDICATED, PENDING, PLANNED, POSSIBLE, PREDICTED,
    SEPARATED, UPDATED.
    """

    data_mode: Literal["REAL", "TEST", "SIMULATED", "EXERCISE"] = FieldInfo(alias="dataMode")
    """Indicator of whether the data is REAL, TEST, EXERCISE, or SIMULATED data:

    REAL:&nbsp;Data collected or produced that pertains to real-world objects,
    events, and analysis.

    TEST:&nbsp;Specific datasets used to evaluate compliance with specifications and
    requirements, and for validating technical, functional, and performance
    characteristics.

    EXERCISE:&nbsp;Data pertaining to a government or military exercise. The data
    may include both real and simulated data.

    SIMULATED:&nbsp;Synthetic data generated by a model to mimic real-world
    datasets.
    """

    event_start_time: datetime = FieldInfo(alias="eventStartTime")
    """
    Timestamp representing the events start time in ISO 8601 UTC format with
    millisecond precision.
    """

    event_type: str = FieldInfo(alias="eventType")
    """The type of event associated with this record.

    Values include: DOCK, UNDOCK, SEPARATION, RENDEZVOUS, PROXIMITY, PEZ, WEZ.
    """

    source: str
    """Source of the data."""

    id: Optional[str] = None
    """Unique identifier of the record, auto-generated by the system."""

    actor_sv_epoch: Optional[datetime] = FieldInfo(alias="actorSVEpoch", default=None)
    """
    State vector epoch time of the actor satellite in ISO 8601 UTC format with
    millisecond precision.
    """

    analysis_duration: Optional[float] = FieldInfo(alias="analysisDuration", default=None)
    """Timespan of the rendezvous analysis in seconds."""

    analysis_epoch: Optional[datetime] = FieldInfo(alias="analysisEpoch", default=None)
    """
    Epoch time of the beginning of the analysis period in ISO 8601 UTC format with
    millisecond precision.
    """

    comp_type: Optional[str] = FieldInfo(alias="compType", default=None)
    """Computation type, values (e.g. PLANARALIGNMENT, LONGITUDE)."""

    context_keys: Optional[List[str]] = FieldInfo(alias="contextKeys", default=None)
    """
    An optional string array containing additional data (keys) representing relevant
    items for context of fields not specifically defined in this schema. This array
    is paired with the contextValues string array and must contain the same number
    of items. Please note these fields are intended for contextual use only and do
    not pertain to core schema information. To ensure proper integration and avoid
    misuse, coordination of how these fields are populated and consumed is required
    during onboarding.
    """

    context_values: Optional[List[str]] = FieldInfo(alias="contextValues", default=None)
    """
    An optional string array containing the values associated with the contextKeys
    array. This array is paired with the contextKeys string array and must contain
    the same number of items. Please note these fields are intended for contextual
    use only and do not pertain to core schema information. To ensure proper
    integration and avoid misuse, coordination of how these fields are populated and
    consumed is required during onboarding.
    """

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Time the row was created in the database, auto-populated by the system."""

    created_by: Optional[str] = FieldInfo(alias="createdBy", default=None)
    """
    Application user who created the row in the database, auto-populated by the
    system.
    """

    cso_details: Optional[List[CloselyspacedobjectTupleResponseItemCsoDetail]] = FieldInfo(
        alias="csoDetails", default=None
    )
    """
    A collection of orbital metrics for the event at the start and end times, and
    the mean values of the primary and secondary objects, as well as the deltas
    between the primary and secondary objects.
    """

    delta_v_tol: Optional[float] = FieldInfo(alias="deltaVTol", default=None)
    """The tolerance value for the DeltaV, in kilometers per second."""

    duration_threshold: Optional[float] = FieldInfo(alias="durationThreshold", default=None)
    """The threshold of the event duration in seconds."""

    event_end_time: Optional[datetime] = FieldInfo(alias="eventEndTime", default=None)
    """
    Timestamp representing the events end time in ISO 8601 UTC format with
    millisecond precision.
    """

    event_interval_coverage: Optional[float] = FieldInfo(alias="eventIntervalCoverage", default=None)
    """
    Percentage of the event interval that is within the plane tolerance specified as
    a percent value between 0 and 100.
    """

    ext_id: Optional[str] = FieldInfo(alias="extId", default=None)
    """Unique identifier of the record from the originating system.

    This field has no meaning within UDL and is provided as a convenience for
    systems that require tracking of an internal system generated ID.
    """

    hohmann_delta_v: Optional[float] = FieldInfo(alias="hohmannDeltaV", default=None)
    """
    The Hohmann DeltaV (kilometers per second) is the minimum delta velocity for the
    in-plane orbit change. The in-plane maneuvers change the semi-major axis
    (perigee and/or apogee). It is the minimum assuming two maneuvers; a lower delta
    velocity is possible with bi-elliptic transfers involving three maneuvers.
    """

    id_actor_sv: Optional[str] = FieldInfo(alias="idActorSV", default=None)
    """Optional ID of the UDL State Vector at epoch time of the actor satellite.

    When performing a create, this id will be ignored in favor of the UDL generated
    id of the actor state vector.
    """

    id_on_orbit1: Optional[str] = FieldInfo(alias="idOnOrbit1", default=None)
    """Unique identifier of the primary satellite on-orbit object, if correlated.

    For rendezvous and proximity operations, this is the target on-orbit object.
    When the secondary object is on the rendezvous capable list, this can be any
    object.
    """

    id_on_orbit2: Optional[str] = FieldInfo(alias="idOnOrbit2", default=None)
    """Unique identifier of the secondary satellite on-orbit object, if correlated.

    For rendezvous and proximity operations, this is the actor. When the primary
    object is a satellite being protected on the neighborhood watch list (NWL), this
    can be any object encroaching on the primary.
    """

    id_target_sv: Optional[str] = FieldInfo(alias="idTargetSV", default=None)
    """Optional ID of the UDL State Vector at epoch time of the target satellite.

    When performing a create, this id will be ignored in favor of the UDL generated
    id of the target state vector.
    """

    inclination_delta_v: Optional[float] = FieldInfo(alias="inclinationDeltaV", default=None)
    """
    The Inclination DeltaV is the minimum delta velocity for the out-of-plane
    change, assuming alignment of the right ascensions measured in kilometers per
    second.
    """

    indication_source: Optional[str] = FieldInfo(alias="indicationSource", default=None)
    """
    Identifies the source of the indication, if the latest event info was manually
    input, not computed.
    """

    lon_tol: Optional[float] = FieldInfo(alias="lonTol", default=None)
    """The tolerance value for the longitude in degrees."""

    max_range: Optional[float] = FieldInfo(alias="maxRange", default=None)
    """
    Maximum range (apogee and perigee differences) within the event interval
    measured in kilometers.
    """

    min_plane_sep_angle: Optional[float] = FieldInfo(alias="minPlaneSepAngle", default=None)
    """
    Minimum angle between the target's position and the projection of the actor's
    position into the target's nominal orbit plane over the event interval measured
    in degrees.
    """

    min_plane_sep_epoch: Optional[datetime] = FieldInfo(alias="minPlaneSepEpoch", default=None)
    """
    Epoch time of the minimum in-plane separation angle occurrence in ISO 8601 UTC
    format with millisecond precision.
    """

    min_range: Optional[float] = FieldInfo(alias="minRange", default=None)
    """
    Minimum range (apogee and perigee differences) within the event interval
    measured in kilometers.
    """

    min_range_analysis_duration: Optional[float] = FieldInfo(alias="minRangeAnalysisDuration", default=None)
    """Timespan of satellites within the range tolerance in seconds."""

    min_range_epoch: Optional[datetime] = FieldInfo(alias="minRangeEpoch", default=None)
    """
    Epoch time of the minimum range occurrence in ISO 8601 UTC format with
    millisecond precision.
    """

    notes: Optional[str] = None
    """Contains other descriptive information associated with an indicated event."""

    num_sub_intervals: Optional[int] = FieldInfo(alias="numSubIntervals", default=None)
    """
    The number of oscillations within the event interval which are within the plane
    tolerance.
    """

    orbit_align_del: Optional[float] = FieldInfo(alias="orbitAlignDel", default=None)
    """
    The change in angle between the angular momentum vectors between the actor and
    target relative to plane orientation in degrees.
    """

    orbit_plane_tol: Optional[float] = FieldInfo(alias="orbitPlaneTol", default=None)
    """
    The tolerance value for the difference in the orbital plane measured in degrees.
    """

    origin: Optional[str] = None
    """
    Originating system or organization which produced the data, if different from
    the source. The origin may be different than the source if the source was a
    mediating system which forwarded the data on behalf of the origin system. If
    null, the source may be assumed to be the origin.
    """

    orig_network: Optional[str] = FieldInfo(alias="origNetwork", default=None)
    """
    The originating source network on which this record was created, auto-populated
    by the system.
    """

    orig_object_id1: Optional[str] = FieldInfo(alias="origObjectId1", default=None)
    """Optional place holder for an OnOrbit ID that does not exist in UDL."""

    orig_object_id2: Optional[str] = FieldInfo(alias="origObjectId2", default=None)
    """Optional place holder for an OnOrbit ID that does not exist in UDL."""

    range_threshold: Optional[float] = FieldInfo(alias="rangeThreshold", default=None)
    """The threshold of the event range in kilometers."""

    range_tol: Optional[float] = FieldInfo(alias="rangeTol", default=None)
    """The tolerance value for the range in kilometers."""

    rel_pos: Optional[List[float]] = FieldInfo(alias="relPos", default=None)
    """
    Indicates the relative position vector of the event occurrence measured in
    kilometers.
    """

    rel_pos_mag: Optional[float] = FieldInfo(alias="relPosMag", default=None)
    """
    Range of closest approach: relative position magnitude, in kilometers, of the
    difference in the physical position between the actor and target objects.
    """

    rel_speed_mag: Optional[float] = FieldInfo(alias="relSpeedMag", default=None)
    """
    Indicates the closure rate specified as a relative velocity magnitude in
    kilometers per second of the difference in the velocities between the actor and
    target objects.
    """

    rel_vel: Optional[List[float]] = FieldInfo(alias="relVel", default=None)
    """
    Indicates the relative velocity vector of the event occurrence measured in
    kilometers per second.
    """

    sat_no1: Optional[int] = FieldInfo(alias="satNo1", default=None)
    """Satellite/catalog number of the target on-orbit primary object."""

    sat_no2: Optional[int] = FieldInfo(alias="satNo2", default=None)
    """Satellite/catalog number of the target on-orbit secondary object."""

    source_dl: Optional[str] = FieldInfo(alias="sourceDL", default=None)
    """The source data library from which this record was received.

    This could be a remote or tactical UDL or another data library. If null, the
    record should be assumed to have originated from the primary Enterprise UDL.
    """

    station_lim_lon_tol: Optional[float] = FieldInfo(alias="stationLimLonTol", default=None)
    """The tolerance value for the optimal longitude for station-keeping in degrees."""

    target_sv_epoch: Optional[datetime] = FieldInfo(alias="targetSVEpoch", default=None)
    """
    State vector epoch time of the target satellite in ISO 8601 UTC format with
    millisecond precision.
    """

    total_delta_v: Optional[float] = FieldInfo(alias="totalDeltaV", default=None)
    """
    The Total DeltaV is the sum of the Hohmann and Inclination DeltaVs measured in
    kilometers per second.
    """


CloselyspacedobjectTupleResponse: TypeAlias = List[CloselyspacedobjectTupleResponseItem]
