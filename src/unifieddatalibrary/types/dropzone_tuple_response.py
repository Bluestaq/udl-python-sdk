# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from typing import List, Optional
from datetime import datetime
from typing_extensions import Literal, TypeAlias

from pydantic import Field as FieldInfo

from .._models import BaseModel

__all__ = ["DropzoneTupleResponse", "DropzoneTupleResponseItem"]


class DropzoneTupleResponseItem(BaseModel):
    classification_marking: str = FieldInfo(alias="classificationMarking")
    """Classification marking of the data in IC/CAPCO Portion-marked format."""

    data_mode: Literal["REAL", "TEST", "SIMULATED", "EXERCISE"] = FieldInfo(alias="dataMode")
    """Indicator of whether the data is REAL, TEST, EXERCISE, or SIMULATED data:

    REAL:&nbsp;Data collected or produced that pertains to real-world objects,
    events, and analysis.

    TEST:&nbsp;Specific datasets used to evaluate compliance with specifications and
    requirements, and for validating technical, functional, and performance
    characteristics.

    EXERCISE:&nbsp;Data pertaining to a government or military exercise. The data
    may include both real and simulated data.

    SIMULATED:&nbsp;Synthetic data generated by a model to mimic real-world
    datasets.
    """

    lat: float
    """WGS84 latitude of the drop zone, in degrees.

    -90 to 90 degrees (negative values south of equator).
    """

    lon: float
    """WGS84 longitude of the drop zone, in degrees.

    -180 to 180 degrees (negative values west of Prime Meridian).
    """

    name: str
    """The name of the drop zone."""

    source: str
    """Source of the data."""

    id: Optional[str] = None
    """Unique identifier of the record, auto-generated by the system."""

    alt_country_code: Optional[str] = FieldInfo(alias="altCountryCode", default=None)
    """
    Specifies an alternate country code for the drop zone if the data provider code
    is not part of an official Country Code standard such as ISO-3166 or FIPS. This
    field will be set to the value provided by the source and should be used for all
    Queries specifying a Country Code.
    """

    alt_country_name: Optional[str] = FieldInfo(alias="altCountryName", default=None)
    """
    Specifies the country name associated with the source provided alternate country
    code.
    """

    approval_date: Optional[datetime] = FieldInfo(alias="approvalDate", default=None)
    """
    The date the drop zone survey was approved, in ISO 8601 UTC format with
    millisecond precision.
    """

    code: Optional[str] = None
    """The type code for the drop zone."""

    country_code: Optional[str] = FieldInfo(alias="countryCode", default=None)
    """The Country Code where the drop zone is located.

    This value is typically the ISO 3166 Alpha-2 two-character country code, however
    it can also represent various consortiums that do not appear in the ISO
    document. The code must correspond to an existing country in the UDL’s country
    API. Call udl/country/{code} to get any associated FIPS code, ISO Alpha-3 code,
    or alternate code values that exist for the specified country code.
    """

    country_name: Optional[str] = FieldInfo(alias="countryName", default=None)
    """The country name of the location for the drop zone."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Time the row was created in the database, auto-populated by the system."""

    created_by: Optional[str] = FieldInfo(alias="createdBy", default=None)
    """
    Application user who created the row in the database, auto-populated by the
    system.
    """

    expiration_date: Optional[datetime] = FieldInfo(alias="expirationDate", default=None)
    """
    The survey expiration date of the drop zone, in ISO 8601 UTC format with
    millisecond precision.
    """

    ext_identifier: Optional[str] = FieldInfo(alias="extIdentifier", default=None)
    """The external identifier assigned to the drop zone."""

    id_site: Optional[str] = FieldInfo(alias="idSite", default=None)
    """The ID of the site associated with the drop zone."""

    last_update: Optional[datetime] = FieldInfo(alias="lastUpdate", default=None)
    """
    Last time the drop zone information was updated, in ISO 8601 UTC format with
    millisecond precision.
    """

    length: Optional[float] = None
    """The length dimension of the drop zone in meters for non-circular drop zones."""

    majcom: Optional[str] = None
    """The Major Command (MAJCOM) responsible for management of the drop zone."""

    nearest_loc: Optional[str] = FieldInfo(alias="nearestLoc", default=None)
    """The nearest reference location to the drop zone."""

    operational_approval_date: Optional[datetime] = FieldInfo(alias="operationalApprovalDate", default=None)
    """
    The approval date for the drop zone by an air drop authority certifying
    operational usage, in ISO 8601 UTC format with millisecond precision.
    """

    origin: Optional[str] = None
    """
    Originating system or organization which produced the data, if different from
    the source. The origin may be different than the source if the source was a
    mediating system which forwarded the data on behalf of the origin system. If
    null, the source may be assumed to be the origin.
    """

    orig_network: Optional[str] = FieldInfo(alias="origNetwork", default=None)
    """
    The originating source network on which this record was created, auto-populated
    by the system.
    """

    point_name: Optional[str] = FieldInfo(alias="pointName", default=None)
    """The name assigned to the drop zone point."""

    radius: Optional[float] = None
    """The radius dimension of the drop zone in meters for circular drop zones."""

    recert_date: Optional[datetime] = FieldInfo(alias="recertDate", default=None)
    """
    The date the drop zone was recertified, in ISO 8601 UTC format with millisecond
    precision.
    """

    remark: Optional[str] = None
    """Remarks concerning the drop zone."""

    source_dl: Optional[str] = FieldInfo(alias="sourceDL", default=None)
    """The source data library from which this record was received.

    This could be a remote or tactical UDL or another data library. If null, the
    record should be assumed to have originated from the primary Enterprise UDL.
    """

    state_abbr: Optional[str] = FieldInfo(alias="stateAbbr", default=None)
    """The US alphabetical code for the state where the drop zone is located."""

    state_name: Optional[str] = FieldInfo(alias="stateName", default=None)
    """The name of the state where the drop zone is located."""

    survey_date: Optional[datetime] = FieldInfo(alias="surveyDate", default=None)
    """
    The date the drop zone survey was performed, in ISO 8601 UTC format with
    millisecond precision.
    """

    updated_at: Optional[datetime] = FieldInfo(alias="updatedAt", default=None)
    """Time the row was last updated in the database, auto-populated by the system."""

    updated_by: Optional[str] = FieldInfo(alias="updatedBy", default=None)
    """
    Application user who updated the row in the database, auto-populated by the
    system.
    """

    width: Optional[float] = None
    """The width dimension of the drop zone in meters for non-circular drop zones."""

    zar_id: Optional[str] = FieldInfo(alias="zarId", default=None)
    """The identifier of the Zone Availability Report (ZAR) for the drop zone."""


DropzoneTupleResponse: TypeAlias = List[DropzoneTupleResponseItem]
